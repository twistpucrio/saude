...
const MapView = (() => {
    ...
    const initMap = (position, mapElementId) => {
        ...
        // Restaura o estado do mapa se houver dados salvos
        restoreMapState();
        NavView.restoreHospitalsFromStorage(MapView.handleHospitalNavClick);
    };
    ...
    const restoreMapState = () => {
        ...
        if (storedData) {
            const { hospitalId, hospitalName, hospitalPosition, mapCenter, zoom } = JSON.parse(storedData);
    
            // Restaura a posição do mapa
            map.setCenter(mapCenter);
            map.setZoom(zoom);
    
            // Cria um marcador temporário para abrir o InfoWindow
            ...
            addClickEventToMarker(marker, hospitalId, hospitalName, hospitalPosition);
    
            // Simula o clique no marcador para abrir o InfoWindow
            google.maps.event.trigger(marker, 'click');
    
            // Limpa o localStorage após restaurar o estado
    ...
    const addRouteOption = (route, index, modo) => {
        ...
        console.log(routeOptionsContainer)
        ...
        if (modo === google.maps.TravelMode.TRANSIT) {
            ...
            details += `
                    <button class="toggle-details-btn" style="display: none;">Detalhes</button>
        ...
        routeOption.addEventListener("click", () => {
            ...
            allOptions.forEach((option) => {
                ...
                // Oculta o botão de detalhes nas outras opções
                option.querySelector('.toggle-details-btn').style.display = 'none';
            });

            // Adiciona a classe 'active' à opção clicada
            routeOption.classList.add("active");

            // Exibe o botão de detalhes apenas na opção ativa
        ...
        // Se for a primeira opção (index === 0), defina como ativa e exiba o botão de detalhes
        if (index === 0) {
            routeOption.classList.add("active"); // Adiciona classe 'active'
            toggleDetailsButton.style.display = 'block'; // Exibe o botão de detalhes
        }
            ...
            if (step.travel_mode === "WALKING") {
                details += `
                    <div class="step-details">
                        ...
                        <div>
                            ${step.instructions} <br> <!-- Instruções -->
    ...
    const highlightRoute = () => {
        ...
        routeRenderers.forEach((renderer, i) => {
            ...
            if (i === activeIndex) {
                ...
                renderer.setOptions({
                    polylineOptions: {
                        strokeColor: "#1e90ff",  // Cor da rota ativa
                        strokeOpacity: 1,
                        strokeWeight: 8,
                    },
                });
                ...
                renderer.setOptions({
                    polylineOptions: {
                        strokeColor: "rgba(12, 122, 255, 0.4)", // Cor das rotas não ativas
                        strokeOpacity: 0.7, // Opacidade para bordas
                        strokeWeight: 6,
                    },
                    zIndex: 1, // Garante que não sobrescrevam a rota ativa
                });
                renderer.setMap(map); // Reanexa o renderizador ao mapa
            }
        });
